...
Line_threshold = (0, 40, -128, 127, -128, 127)  # 黑线阈值
A_port_width = 85  # A点色块宽度阈值
Wait_port_width = 160  # 等停指示色块宽度阈值

UpROI = (0, 80, 240, 20)  # 向上识别ROI
...
while (True):
    ...
    # 巡线
    LineBlobs = img.find_blobs([Line_threshold], roi=DefaultROI, x_stride=20, pixels_threshold=PixelsThreshold,
                               merge=True)
    if LineBlobs:
        line_blobs_num = 0
        for lb in LineBlobs:
        ...
        if line_blobs_num == 1:  ##非岔路口
            if lb["w"] > Wait_port_width:  ###等停点
                # print("Waiting Port")
                data = 'S' + str(int(lb["x"] + 0.5 * lb["w"])) + '\n'
                ser.write(data.encode())

                img.draw_string(10, 10, "Waiting Port", scale=1.0, color=(255, 255, 255), thickness=1)

            elif lb["w"] > A_port_width:  ###A点或岔路过渡
                UpBlobs = img.find_blobs([Line_threshold], roi=UpROI, x_stride=20, pixels_threshold=PixelsThreshold,
                                         merge=True)
                if UpBlobs:
                    up_line_blobs_num = 0
                    for ub in UpBlobs:
                        up_line_blobs_num += 1
                        img.draw_rectangle(ub["x"], ub["y"], ub["x"] + ub["w"], ub["y"] + ub["h"], color=(0, 0, 255),
                                           thickness=1)
                    ...
                    img.draw_string(10, 100, str(int(lb["x"] + 0.5 * lb["w"])), scale=1.0, color=(255, 255, 255),
                                    thickness=1)
